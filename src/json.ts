...
import type {CommanderProgramLike} from './types.js'
...
/**
 * Render Commander help for every visible command node in depth-first pre-order.
 *
 * The blocks are still generated by Commander via `helpInformation()`; this
 * helper only adds full-path headings and walks the command tree.
 */
export const deepHelp = (program: CommanderProgramLike): string => {
  const sections: string[] = []
  collectHelpSections(program, '', sections)
  return sections.join('\n')
}

const collectHelpSections = (command: CommanderProgramLike, parentPath: string, sections: string[]) => {
  const commandPath = commandPathFor(command, parentPath)
  sections.push(`=== ${commandPath} ===\n${command.helpInformation()}`)
  command.commands?.filter(isVisibleCommand).forEach(child => collectHelpSections(child, commandPath, sections))
}

const commandPathFor = (command: CommanderProgramLike, parentPath: string) => {
  const name = command.name() || 'program'
  return parentPath ? `${parentPath} ${name}` : name
}

const isVisibleCommand = (command: CommanderProgramLike) => command.hidden !== true && command._hidden !== true
