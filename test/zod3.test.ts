...
test('enum input', async () => {
  ...
  await expect(run(router, ['foo', 'cc'])).rejects.toMatchInlineSnapshot(`
    CLI exited with code 1
      Caused by: CliValidationError: error: command-argument value 'cc' is invalid for argument 'string'. Invalid enum value. Expected 'aa' | 'bb', received 'cc'
  `)
})
...
test('boolean input', async () => {
  ...
  await expect(run(router, ['foo', 'a'])).rejects.toMatchInlineSnapshot(`
    CLI exited with code 1
      Caused by: CliValidationError: error: command-argument value 'a' is invalid for argument 'boolean'. Expected boolean, received string
  `)
})
...
test('literal input', async () => {
  ...
  await expect(run(router, ['foo', '3'])).rejects.toMatchInlineSnapshot(`
    CLI exited with code 1
      Caused by: CliValidationError: error: command-argument value '3' is invalid for argument 'number'. Invalid literal value, expected 2
  `)
})
...
test('regex input', async () => {
  ...
  await expect(run(router, ['foo', 'goodbye xyz'])).rejects.toMatchInlineSnapshot(`
    CLI exited with code 1
      Caused by: CliValidationError: error: command-argument value 'goodbye xyz' is invalid for argument 'value'. Invalid
  `)
})
...
test('number array input', async () => {
  ...
  await expect(run(router, ['test', '1', 'bad'])).rejects.toMatchInlineSnapshot(`
    CLI exited with code 1
      Caused by: CliValidationError: error: command-argument value 'bad' is invalid for argument 'parameter_1'. Expected number, received string
  `)
})
...
test('boolean array input', async () => {
  ...
  await expect(run(router, ['test', 'true', 'bad'])).rejects.toMatchInlineSnapshot(`
    CLI exited with code 1
      Caused by: CliValidationError: error: command-argument value 'bad' is invalid for argument 'parameter_1'. Expected boolean, received string
  `)
})
...
test('record input', async () => {
  ...
  await expect(run(router, ['test', '--json', '{"foo": "x"}'])).rejects.toMatchInlineSnapshot(`
    CLI exited with code 1
      Caused by: CliValidationError: error: option '--json <json>' argument '{"foo":"x"}' is invalid. Expected number, received string → at foo
  `)
})
