...
- 2026-06-11 (night, second review round): addressed the remaining findings from the second 🤖 review on PR #205 (the first round was handled in c91a1b2). (1) `wrap` in src/typebox/index.ts no longer wraps classes (`Type.Base`, `DecodeBuilder`, `EncodeBuilder`) - arrow wrappers have no [[Construct]], so `class DateType extends Type.Base<Date>` threw "is not a constructor". Classes are detected via their non-writable `prototype` descriptor. (2) `attachStandardSchema` now returns frozen (non-extensible) schemas as-is instead of throwing - `Settings.Set({immutableTypes: true})` makes Memory.Create freeze every schema, which made every wrapped builder throw at defineProperty. Graceful degradation documented in the README typebox section. (3) Added `TTemplateLiteral` and `TCyclic` to the declare-module list in src/typebox/standard.ts (runtime already attached `~standard`; the type level rejected them). `TCodec` deliberately NOT added: it's a type alias (`Type & {'~codec': ...}`), not an interface, so it can't be augmented - and it doesn't need to be, since `Type.Decode`/`Type.Encode` results carry `~standard` at runtime (verified) and type-level via the wrapped base type's augmentation. (4) Nit: wrapped builders now preserve `fn.name` (`Type.String.name === 'String'`; `Type.Object.name === '_Object_'`, upstream's internal anti-collision name). Skipped nits, deliberately: double-jsdoc-takes-first (edge case, behavior is sane) and the jsonSchema input→output fallback leniency in src/json-schema.ts. All five behaviors covered by new tests in test/typebox.test.ts (written failing-first). The cyclic test runs through `--json` because root-level `{$defs, $ref}` schemas can't flatten into CLI flags - a pre-existing trpc-cli JSON-schema limitation unrelated to `~standard`.
