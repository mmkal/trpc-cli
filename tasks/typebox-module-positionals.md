...
- 2026-06-12 (overload support, implemented on the base branch): TS function overloads now use the *first overload signature* as the command's schema (full decision log in tasks/typebox-module-commands.md). For positional functions this means the first signature drives the positionals, the trailing flags object, and (via its jsdoc) the description - the widened implementation signature (`name: any, options?: any`) no longer wins and no longer triggers the "type `any` can't be a positional" error. Pinned by the 'overloaded multi-parameter functions use the first overload signature' test.
