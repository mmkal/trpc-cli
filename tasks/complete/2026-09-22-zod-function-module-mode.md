...
- 2026-09-22: positionals are named after the `.implement((name, options) => ...)` callback's parameters (`extractImplementParamNames` + `withParameterNames` in src/module-commands.ts). zod tuple items carry no names, so a bare `z.string()` showed as `<parameter_1>`. The tuple is converted to JSON schema once, scalar items without a `.meta({title})` get `title` from the callback, and the result is handed to `.input()` as a plain schema with a `~standard` that validates via the original tuple (same shape as the typebox flow). Note zod stamps its `toJSONSchema` output with a non-configurable `~standard` claiming vendor `zod`, hence the shallow copy.
