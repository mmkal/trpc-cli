...
Done. TypeBox Standard Schema wrappers now map to CLI arguments through `~standard.jsonSchema`, TypeBox integration coverage is in place, README usage notes document the wrapper requirement, and the full test suite passes.
...
- [x] Add TypeBox as an optional peer/dev dependency if the real package is required for tests. _Added `typebox` as a dev dependency for integration coverage; no runtime peer is needed because trpc-cli consumes the Standard Schema shape structurally._
